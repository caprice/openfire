...
<%  // Get parameters //
    ...
    String plate = ParamUtils.getParameter(request,"plate");
    ...
    if (create) {
        ...
        if (errors.size() == 0) {
            try {
                User newUser = webManager.getUserManager().createUser(username, password, name, email, plate);
                ...
                if (!SecurityAuditManager.getSecurityAuditProvider().blockUserEvents()) {
                    // Log the event
                    webManager.logEvent("created new user "+username, "name = "+name+", email = "+email+", plate = "+plate+", admin = "+isAdmin);
                }
...
<%  if (!errors.isEmpty()) { %>
    ...
    <tbody>
        <tr>
            ...
            <% } else if (errors.get("plate") != null) { %>
                <fmt:message key="user.create.invalid_plate" />
...
<form name="f" action="user-create.jsp" method="get">
	...
	<div class="jive-contentBox">
		...
		<tr>
			<td width="1%" nowrap>
				<label for="emailtf"><fmt:message key="user.create.plate" />:</label> <%= UserManager.getUserProvider().isPlateRequired() ? "*" : "" %></td>
			<td width="99%">
				<input type="text" name="plate" size="30" maxlength="50" value="<%= ((plate!=null) ? plate : "") %>"
				 id="platetf">
			</td>
		</tr>		
		<tr>
