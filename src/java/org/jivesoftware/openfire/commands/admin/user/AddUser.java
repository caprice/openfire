...
public class AddUser extends AdHocCommand {
	...
	public void execute(SessionData sessionData, Element command) {
        ...
        if (UserManager.getUserProvider().isPlateRequired()) {
            note.addAttribute("type", "error");
            note.setText("No plate was specified.");
            return;
        }

        try {
            UserManager.getInstance().createUser(account.getNode(), password, name, email, plate);
        }
	...
	protected void addStageInformation(SessionData data, Element command) {
        ...
        field = form.addField();
        field.setType(FormField.Type.text_single);
        field.setLabel("Plate");
        field.setVariable("plate");
